...
class WooCrawlSpider(scrapy.Spider):
    ...
    def clean_html_description(self,html_content):
        ...
        # 2. 移除不需要的交互/媒体标签
        # 移除脚本、样式、图片、视频、嵌入框架等
        for tag in soup.find_all(['script', 'style', 'img', 'video', 'iframe', 'canvas', 'svg', 'noscript']):
            tag.decompose()

        # 3. 清理标签属性 (可选：防止原站的样式干扰你的站点)
        # 只保留必需的标签，移除所有 class, id, style 等属性
        allowed_tags = ['p', 'br', 'ul', 'ol', 'li', 'strong', 'b', 'em', 'i', 'h1', 'h2', 'h3', 'h4']
        for tag in soup.find_all(True):
            if tag.name not in allowed_tags:
                # 如果是不在允许列表里的标签（如 div, span），保留内容但移除标签外壳
                tag.unwrap()
            else:
                # 清除所有属性（class, style, id 等）
                tag.attrs = {}

        # 4. 格式化输出：去除多余空行，返回字符串
        # 使用 str(soup) 会保留 HTML 结构，如果只要纯文本则用 soup.get_text()
        ...
        return cleaned_html
    def __init__(self, domain=None, category="未知分类", config_file=None, mode="prod",*args, **kwargs):
        super().__init__(*args, **kwargs)
        # 新增 mode 参数：dev (测试) 或 prod (生产)
        self.mode = mode
        self.item_count = 0
        self.max_items = 10 if mode == "dev" else 999999
        # 动态传入的域名和分类
    ...
    def parse_meta_currency(self, response):
        if response.status == 200:
            try:
                ...
                if sitemap_links:
                    if self.mode == "dev":
                        sitemap_links = sitemap_links[:1]
                        self.logger.info("--- [DEV模式] 仅解析第一个子站点地图 ---")
    ...
    def parse_product_sitemap(self, response):
        ...
        # 如果是开发模式，只截取前 10 个 URL
        if self.mode == "dev":
            product_urls = product_urls[:self.max_items]
            self.logger.info(f"--- 开发模式：仅提取前 {len(product_urls)} 条数据进行测试 ---")
        # 去重并发起详情页请求
        valid_count = 0
    ...
    def parse_product_detail(self, response):
        ...
        try:
            ...
            raw_desc_list = response.xpath(self.selectors["description"]).getall()
            full_html_desc = "".join(raw_desc_list)
            # Description：必须用 getall() 合并多个文本节点
            description = self.clean_html_description(full_html_desc)
            # 主图
            ...
            if current_price <= 0:
                self.logger.warning(f"🚫 过滤：价格为 0 或无效 ({price_clean}) - URL: {response.url}")
                return  # 丢弃该条数据，不执行 yield

            # 2. 检查图片是否为空
            if not images or images.strip() == "":
                self.logger.warning(f"🚫 过滤：图片链接为空 - URL: {response.url}")
                return  # 丢弃该条数据
